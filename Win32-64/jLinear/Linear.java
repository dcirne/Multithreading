...
public class Linear {
	...
	public static void main(String [] args) {
        ...
        if (("/").equals(System.getProperties().getProperty("file.separator"))) {
            FileName = "Java.Posix.Linear.csv";
        } else {
            FileName = "Java.Windows.Linear.csv";
        }
		...
		for (int iteraction = 0; iteraction < numberIteractions; iteraction++) {
            ...
            IncrementCounter incrementCounter = new IncrementCounter(numberOfCounterIncrements, timeTracker);
			...
			ConsumeEquationResults consumeEquationResults = new ConsumeEquationResults(equationCalculator);
            for (int i = 0; i < numberOfEquationPoints; i++) {
                produceEquationResult.run(i);
                consumeEquationResults.run();
            }
...
// Increments a counter "numIncs" times. 
...
class EquationCalculator {
    ...
    public EquationCoordinate getEquationResult() {
        return equationCord;
	}
    
	public void calculateEquation(Integer i) {
...
class ConsumeEquationResults {
	EquationCalculator equationCalculator;
	
	ConsumeEquationResults(EquationCalculator equationCalculator) {
