...
public class Threads {
	...
	public static void main(String [] args) {
        ...
        if (("/").equals(System.getProperties().getProperty("file.separator"))) {
            FileName = "Java.Posix.Threads.csv";
        } else {
            FileName = "Java.Windows.Threads.csv";
        }
		...
		for (int iteraction = 0; iteraction < numberIteractions; iteraction++) {
			...
			thrds.add(new Thread(new IncrementCounter(numberOfCounterIncrements, timeTracker)));
			thrds.add(new Thread(new ReplicateFile(dirNumber, numberOfOutputFiles, timeTracker)));
			thrds.add(new Thread(new ConsumeEquationResults(equationCalculator, timeTracker)));
...
class IncrementCounter implements Runnable {
	...
	public synchronized void run() {
        ...
        Long i = 0L, temp;
        do {
            temp = Threads.counter;
            temp = temp + increment;
            temp = temp - decrement;
            Threads.counter = temp;
            i = i + 1;
        } while (i < numIncs);
